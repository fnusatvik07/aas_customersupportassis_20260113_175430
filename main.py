...
CORS_ORIGINS = [
    ...
    f"http://localhost:8021", 
    f"http://localhost:8022",
    f"http://127.0.0.1:8020",
    f"http://127.0.0.1:8021",
    f"http://127.0.0.1:8022"
]
...
2. Use AskUserQuestion to gather necessary details from customers to understand their issues
3. Analyze inquiry complexity and determine if escalation to human agents is needed
4. Track all customer interactions, response times, and resolution times using TodoWrite and file operations
5. Maintain a log of inquiries with timestamps, issue categories, and resolution status
6. Generate reports on resolution times and support metrics

Escalation criteria:
- Technical issues beyond standard troubleshooting
- Requests for refunds or account modifications
- Complaints requiring managerial intervention
- Legal or compliance matters
- Customer dissatisfaction with initial response
...
- Analyze patterns in customer inquiries to identify common issues
- Track metrics: first response time, resolution time, escalation rate

Always maintain detailed records and provide analytical insights on support performance.""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8019",
        "frontend_url": f"http://localhost:8020",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8019")
    print(f"[FRONTEND] Frontend URL: http://localhost:8020")  
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8019/docs")
    print(f"[HEALTH] Health: http://localhost:8019/health")
